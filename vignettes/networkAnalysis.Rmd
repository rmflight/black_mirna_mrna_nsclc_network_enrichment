...
We will use the cutoff for `combined_score` of 400 (the default in STRING itself), and then filter down to just those things with co-expression, experimental, and database evidence.

```{r filterData}
minScores <- c(combined_score=400, coexpression=1, experimental=1, database=1)
passesScore <- rep(TRUE, nrow(stringData))
