...
For a student application, generate neighborhood for just EIF4B / RAB8A.

```{r eifrab}
eifrabNodes <- nodeInfo$nodeID[nodeInfo$SYMBOL %in% c("EIF4B", "RAB8A")]
...
eifInfo <- nodeInfo[(nodeInfo$nodeID %in% nodes(eifGraph)),]
...
eifNew <- eifMatrix
eifNew <- rbind(eifNew, matrix(0, nrow=5, ncol=ncol(eifNew)))
eifNew <- cbind(eifNew, matrix(0, nrow=nrow(eifNew), ncol=5))
...
```{r newEIFRAB}
eifrabNodes <- eifNewInfo$nodeID[eifNewInfo$SYMBOL %in% c("EIF4B", "RAB8A")]
