...
ebComm <- edge.betweenness.community(commGraph)
```

```{r ebComms}
ebAll <- split(ebComm$names, ebComm$membership)
ebAll <- ebAll[order(sapply(ebAll, length), decreasing=TRUE)]
```

```{r selectEB}
selectNodes(cw, ebAll[[1]], preserve.current.selection=FALSE)
selectNodes(cw, ebAll[[2]], preserve.current.selection=FALSE)
selectNodes(cw, ebAll[[3]], preserve.current.selection=FALSE)
```

### Label Propagation

```{r labelProp}
lpComm <- label.propagation.community(commGraph)
lpAll <- split(lpComm$names, lpComm$membership)
lpAll <- lpAll[order(sapply(lpAll, length), decreasing=TRUE)]
```

```{r selectLP}
selectNodes(cw, lpAll[[1]], preserve.current.selection=FALSE)
selectNodes(cw, lpAll[[2]], preserve.current.selection=FALSE)
selectNodes(cw, lpAll[[3]], preserve.current.selection=FALSE) # one with AKT1,3
selectNodes(cw, lpAll[[4]], preserve.current.selection=FALSE)
```
